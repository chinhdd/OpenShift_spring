...
public class MomentumBackpropagation extends BackPropagation {
    ...
    private static final double MAX_WEIGHT = 100000;
    ...
    public void updateNeuronWeights(Neuron neuron) {
        double maxWeightValue = MAX_WEIGHT;
        for (Connection connection : neuron.getInputConnections()) {
            ...
            if (this.isInBatchMode() == false) {
                ...
                if (Math.abs(weight.value) > maxWeightValue) {
                    maxWeightValue = Math.abs(weight.value);
                }
            } else { // otherwise, sum the weight changes and apply them after at the end of epoch
                weight.weightChange += weightChange;
            }
        }
        if (maxWeightValue > MAX_WEIGHT) {
            double ratio = MAX_WEIGHT * 0.5 / maxWeightValue;//ratio should be smaller than 1.00
            for (Connection connection : neuron.getInputConnections()) {
               Weight weight = connection.getWeight();
               weight.value = weight.value * ratio;
//               if (Math.abs(weight.value) < MIN_WEIGHT) {
//                  weight.value = Math.signum(weight.value) * MIN_WEIGHT;
//               }
            }
         }
    }
